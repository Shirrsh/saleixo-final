      ...
      <div className="container mx-auto px-4">
        <div className="text-center mb-16 max-w-4xl mx-auto">
          ...
          <div className="space-y-4 text-left md:text-center">
            <p className="text-lg md:text-xl text-muted-foreground animate-fade-in animate-delay-200 leading-relaxed">
              <strong className="text-foreground">Salixo</strong> is a team of master photographers, editors & stylists, each bringing over <strong className="text-primary">two decades of specialized expertise</strong> in luxury product photography.
            </p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-6xl mx-auto">
          {valueProps.map((benefit, index) => (
            <Card key={benefit.id} className={`border-border hover:shadow-lg transition-all duration-500 hover-lift animate-scale-in animate-delay-${(index + 2) * 100} group`}>
              <CardHeader>
                <div className="flex items-center gap-4">
                  <div className="group-hover:scale-110 transition-transform duration-300 animate-float" style={{ animationDelay: `${index * 0.3}s` }}>
                    {benefit.icon && iconMap[benefit.icon] ? iconMap[benefit.icon] : defaultIcons[index % defaultIcons.length]}
                  </div>
